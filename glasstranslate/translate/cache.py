"""Thread-safe LRU cache for translations keyed on ``(text, src, tgt)``.

The pipeline consults the cache before every translation call so unchanged
segments never hit the model twice.  Optional JSON persistence lets the cache
survive restarts.
"""
...
class TranslationCache:
    """Least-recently-used mapping ``(text, src, tgt) -> translation``.
    ...
    def save(self, path: Union[str, Path]) -> Path:
        ...
        with self._lock:
            entries = [
                {"text": t, "src": s, "tgt": g, "translation": v}
                for (t, s, g), v in self._data.items()
            ]
        payload = {"version": 1, "max_size": self.max_size, "entries": entries}
    ...
    def load(self, path: Union[str, Path]) -> int:
        ...
        with self._lock:
            for entry in entries:
                try:
                    key = (str(entry["text"]), str(entry["src"]), str(entry["tgt"]))
